...
<?php
    ...
    $dataProvider = new ActiveDataProvider([
        'query' => Penztarca::find()
            ->where(['felhasznalo' => Yii::$app->user->id, 'torolt' => 0, 'megtakaritas' => 0])
    ...
    if (count($megtakaritasok) > 0)  {
        ...
        $dataProvider = new ActiveDataProvider([
            'query' => Penztarca::find()
                ->where(['felhasznalo' => Yii::$app->user->id, 'torolt' => 0, 'megtakaritas' => 1])
                ->orderBy(['nev' => SORT_ASC]),
        ]);
        ...
        foreach ($devizak as $deviza) {
            ...
            echo "<div><H3>Összesen: ".
                Yii::$app->formatter->asCurrency(
                    Penztarca::getOsszEgyenleg($deviza->deviza, $ig, true), $deviza->deviza
            )."</H3></div>";
